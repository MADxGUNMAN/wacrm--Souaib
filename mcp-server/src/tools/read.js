...
export function registerReadTools(server, client) {
    server.registerTool('whoami', {
        title: 'Who am I',
        description: 'Verify the API key and show which wacrm account it is bound to and what scopes it carries. Call this first to discover what actions are possible.',
        inputSchema: {},
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'Who am I' }),
    }, handle(async () => jsonResult(await client.me())));
    server.registerTool('list_contacts', {
        title: 'List contacts',
        description: 'List contacts in the CRM, newest first. Optionally filter by a free-text search (matches name or phone) or by a tag id. Results are paginated: pass the returned next_cursor to fetch the next page.',
        inputSchema: {
        ...
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'List contacts' }),
    ...
    server.registerTool('get_contact', {
        ...
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'Get contact' }),
    ...
    server.registerTool('list_conversations', {
        ...
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'List conversations' }),
    ...
    server.registerTool('get_conversation', {
        ...
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'Get conversation' }),
    }, handle(async ({ id }) => jsonResult(await client.getConversation(id))));
    server.registerTool('list_messages', {
        title: 'List messages',
        description: 'List the messages in a conversation, newest first. Each message includes its direction (inbound/outbound), delivery status, and content. Paginated.',
        inputSchema: {
            conversation_id: z.string().describe('The conversation to read messages from.'),
            limit: z.number().int().min(1).max(100).optional().describe('Page size, 1–100 (default 50).'),
            cursor: z.string().optional().describe('Opaque pagination cursor.'),
        },
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'List messages' }),
    }, handle(async ({ conversation_id, limit, cursor }) => jsonResult(await client.listConversationMessages(conversation_id, { limit, cursor }))));
    server.registerTool('get_broadcast', {
        title: 'Get broadcast status',
        description: 'Read a broadcast campaign by id — its status and delivered / read / rejected counts. Use this to poll progress after launching one.',
        inputSchema: {
            id: z.string().describe('Broadcast id.'),
        },
        annotations: Object.assign(Object.assign({}, READ_ONLY), { title: 'Get broadcast status' }),
