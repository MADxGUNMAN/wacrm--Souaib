...
export function registerBroadcastTools(server, client) {
    server.registerTool('send_broadcast', {
        ...
        var { confirm } = _a, body = __rest(_a, ["confirm"]);
        if (confirm !== true) {
            return errorResult('Refusing to send: confirm must be true. This launches a mass broadcast to ' +
