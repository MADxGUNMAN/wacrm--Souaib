...
export function AiConfig() {
  ...
  const fetchModels = useCallback(async (currentProvider: AiProvider, keyToUse?: string) => {
  ...
  // Fetch models whenever provider or the API key changes (if they blur/stop typing, but let's just do provider for now, and after save)
  useEffect(() => {
    // Only fetch if we are configured or they have entered a key.
    // If not, fetchModels handles it by returning [] if no key is present in DB and no key passed.
    fetchModels(provider, keyEdited ? apiKey.trim() : undefined);
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    {Object.entries(PROVIDER_LABEL).map(([val, label]) => (
              ...
              <div className="space-y-2">
                ...
                {loadingModels ? (
                  <div className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm text-muted-foreground opacity-50">
                    Loading models...
                  ...
                  <Select value={model} onValueChange={(v) => setModel(v || '')} disabled={disabled}>
                    ...
                    <SelectContent className="w-auto min-w-[var(--anchor-width)] max-h-[400px]">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger id="ai-handoff">
                  <SelectValue placeholder={t('handoffQueue')}>
