...
export function MembersTab() {
  ...
  async function handleToggleSuspend(member: Member) {
    ...
    setMembers((prev) =>
      prev.map((m) =>
        m.user_id === member.user_id ? { ...m, is_active: nextStatus } : m,
      ),
    );
    try {
      const res = await fetch(`/api/account/members/${member.user_id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(
        nextStatus
          ...
          : t('suspendedToast', { name: member.full_name || t('unnamed') })
      );
    } catch (err) {
      setMembers((prev) =>
        prev.map((m) =>
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
                  ...
                  <div className="flex min-w-0 flex-1 items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {!member.is_active && (
                          <Badge className="bg-red-500/10 text-red-500 border-red-500/20 text-[10px] uppercase tracking-wide">
                            {t('suspendedBadge')}
                          </Badge>
                        )}
                      </div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {Object.entries(member.permissions)
                            ...
                            .map(([key]) => (
                              <span
                                key={key}
                                className="inline-flex items-center rounded bg-primary/10 px-1.5 py-0.5 text-[10px] font-medium text-primary border border-primary/20 capitalize"
                              >
                                {key}
                              </span>
                            ))}
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    {canManageMembers && !isOwnerRow && (
                      <Button
                        ...
                        title="Edit Section Access Permissions"
                        className="h-8 px-2 border-border text-muted-foreground hover:text-foreground hover:bg-muted"
                      >
                        <Shield className="size-3.5" />
                        <span className="hidden md:inline ml-1 text-xs">Permissions</span>
                      </Button>
                    )}

                    {/* Role badge — static since all non-owners are members */}
                    <span
                    ...
                    {canManageMembers && !isOwnerRow && !isSelf && (
                      <Button
                        ...
                        className={member.is_active 
                          ...
                          : "border-emerald-500/40 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 hover:text-emerald-400"}
                      >
      ...
      <CreateMemberDialog
        ...
        onCreated={() => void loadEverything()}
      />

      <EditPermissionsDialog
        member={editingPermissionsMember}
        open={!!editingPermissionsMember}
        onOpenChange={(open) => {
          if (!open) setEditingPermissionsMember(null);
        }}
        onUpdated={() => void loadEverything()}
      />
    </section>
  );
}
