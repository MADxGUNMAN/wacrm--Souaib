...
export function LandingFooter({ settings, legalPages }: LandingFooterProps) {
  ...
  const copyrightFull =
    ...
    `© ${new Date().getFullYear()} Junkies Coder. All rights reserved.|||Made with ❤️ in India`;
  const parts = copyrightFull.includes('|||')
    ? copyrightFull.split('|||')
    : [copyrightFull, 'Made with ❤️ in India'];
  const copyrightLeft =
    parts[0] ||
    `© ${new Date().getFullYear()} Junkies Coder. All rights reserved.`;
  const copyrightRight = parts[1] || 'Made with ❤️ in India';
  const copyrightLeftLink = parts[2]?.trim()
    ? parts[2].trim().startsWith('http') || parts[2].trim().startsWith('/')
      ? parts[2].trim()
      : `https://${parts[2].trim()}`
    : null;
  const copyrightRightLink = parts[3]?.trim()
    ? parts[3].trim().startsWith('http') || parts[3].trim().startsWith('/')
      ? parts[3].trim()
      : `https://${parts[3].trim()}`
    : null;
    ...
    <footer className="w-full border-t border-slate-200 bg-white px-6 py-12">
      ...
      >
        ...
        <div className="col-span-1">
          ...
          <p className="mb-6 text-sm text-slate-600">
            {settings?.site_description ||
              'As an official Meta Business Partner, we deliver secure, reliable, and enterprise-grade WhatsApp solutions trusted by businesses globally.'}
          </p>
        ...
        {settings?.footer_links?.map((column, colIdx) => (
          <div
            key={colIdx}
            className="flex flex-col gap-3 text-sm text-slate-600 lg:justify-self-center"
          >
        ...
        {settings?.show_newsletter && (
          <div className="col-span-1 flex flex-col gap-3 text-sm text-slate-600">
            <h4 className="mb-2 font-semibold text-slate-900">
              Subscribe to newsletter
            </h4>
            <p className="mb-1 text-xs leading-relaxed text-slate-500">
              Get the latest updates and product announcements directly to your
              inbox.
            </p>
            <form
            ...
            >
              ...
              <button
                type="submit"
                disabled={newsletterLoading}
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#25D366] px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-[#1eae53] disabled:opacity-50"
              >
                {newsletterLoading && (
            ...
            {newsletterMessage && (
              <p
                className={`mt-1 text-xs ${newsletterMessage.type === 'success' ? 'text-green-600' : 'text-red-600'}`}
              >
      ...
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 border-t border-slate-200 pt-6 md:flex-row">
        ...
        {copyrightRightLink ? (
          ...
          <p className="text-sm font-medium text-slate-500">{copyrightRight}</p>
        )}
      </div>
    </footer>
  );
