...
export function ContactDetailView({
  ...
  // Selected tags always visible; unselected limited to this many initially
  const UNSELECTED_TAGS_LIMIT = 12;

  // Notes tab
  const [notes, setNotes] = useState<ContactNote[]>([]);
  const [loadingNotes, setLoadingNotes] = useState(false);

  // Custom fields (merged into Details tab)
  ...
  async function handleSendTemplate(
    ...
    try {
      const res = await fetch('/api/whatsapp/send', {
        ...
        // Shared builder, not a hand-written object. This payload used to
        // list `body`, `headerText` and `buttonParams` and nothing else,
        // which dropped the carousel cards the picker had just collected —
        // so a carousel template failed from here with "Card 1 needs a
        // media link or id at send time" and worked from the inbox. Header
        // media, offers, location pins, named params and order references
      ...
      if (!res.ok) {
        // Same translation as the inbox path — a bare "HTTP 502" here was
        // just as opaque. See meta-send-errors.ts for why the status alone
        // is misleading.
  ...
  const emailLabel = emailRow?.field_name || t('email');

  const companyVisible = companyRow?.visible !== false;
  const companyLabel = companyRow?.field_name || t('company');
              ...
              >
                ...
                >
                  <div className="space-y-3">
                    ...
                    {/* Dynamic custom fields — inline in Details */}
                    {pureCustomFields.length > 0 && (
                      <div className="border-border space-y-3 border-t pt-3">
                        {pureCustomFields.map((field) => (
                ...
                >
                  <div className="space-y-3">
                    ...
                    {allTags.length === 0 ? (
                      ...
                      (() => {
                        // Split tags: selected always visible, unselected limited
                        const selectedTags = allTags.filter((t) =>
                          contactTagIds.includes(t.id)
                        );
                        const unselectedTags = allTags.filter(
                          (t) => !contactTagIds.includes(t.id)
                        );
                        const hiddenCount = Math.max(
                          0,
                          unselectedTags.length - UNSELECTED_TAGS_LIMIT
                        );
                        const visibleUnselected = showAllTags
                          ? unselectedTags
                          : unselectedTags.slice(0, UNSELECTED_TAGS_LIMIT);
                        const visibleTags = [
                          ...selectedTags,
                          ...visibleUnselected,
                        ];
