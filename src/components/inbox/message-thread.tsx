...
export function MessageThread({
  ...
  return (
    ...
    // Issue #257.
    <div className={cn("flex min-w-0 min-h-0 flex-1 flex-col", DOODLE_BG_CLASSES)}>
      {/* Header — solid card surface sits on top of the doodle so the
          name/avatar/dropdowns stay legible. */}
      <div className={cn("flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4", isFloating && "drag-handle")}>
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <Badge
              variant="outline"
              className={cn(
                "ml-1 hidden gap-1 border-border text-[10px]",
                isFloating ? "hidden" : "sm:inline-flex sm:ml-2",
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                  "inline-flex shrink-0 whitespace-nowrap items-center justify-center h-7 gap-1 px-2 text-xs rounded-md hover:bg-muted",
                  currentStatus?.color ?? "text-muted-foreground",
                  isFloating && "px-1 w-7" // Make it a square icon button when floating
                )}>
                <span className={cn(isFloating && "hidden")}>
                  {currentStatus ? t(`status${currentStatus.label}`) : t("status")}
                </span>
                {isFloating ? (
                  <div className="h-3 w-3 rounded-full border-[2px] border-current" title={currentStatus ? t(`status${currentStatus.label}`) : t("status")} />
                ) : (
          ...
          {isOwner && (
            <DropdownMenu>
              <DropdownMenuTrigger className={cn(
                "inline-flex shrink-0 whitespace-nowrap h-7 items-center gap-1 rounded-md px-2 text-xs text-muted-foreground hover:bg-muted hover:text-foreground",
                isFloating && "px-0 w-7 justify-center" // Make it a square icon button when floating
              )} title={assignLabel}>
                <UserPlus className="h-3.5 w-3.5" />
                <span className={cn(isFloating ? "hidden" : "hidden sm:inline")}>{assignLabel}</span>
      ...
      <div 
        ref={scrollRef} 
        className="flex-1 overflow-y-auto px-4 py-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 hover:[&::-webkit-scrollbar-thumb]:bg-muted-foreground/40"
      >
        {loading ? (
