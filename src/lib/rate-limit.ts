...
export const RATE_LIMITS = {
  ...
  /** Broadcast test send, per user. Deliberately tighter than `send`:
   *  every call is a real, billable WhatsApp message to a number the
   *  operator typed by hand, and the legitimate pattern is "send one,
   *  look at my phone, adjust, send another." 10/min absorbs that plus a
   *  fat-fingered double-click, while stopping the test box being used
   *  as an unmetered single-send channel that bypasses the broadcast
   *  budget entirely. */
  testSend: { limit: 10, windowMs: 60_000 },
} as const;
