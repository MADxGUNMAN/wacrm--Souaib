...
/**
 * Legal shape of a `var_key`.
 *
 * Exported because three separate places have to agree on it: this
 * validator, the builder input that filters keystrokes, and the node
 * reference / AI-prompt docs that tell authors the rule. A fourth
 * hand-written copy of the pattern is how documentation starts lying.
 *
 * The rule is dictated by `interpolateVars` in the engine, which only
 * matches `[a-zA-Z0-9_]` inside `{{vars.…}}` — anything else is
 * unreachable once stored.
 */
export const VAR_KEY_PATTERN = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
...
/**
 * `var_key` is optional on send_buttons / send_list, but a malformed one
 * fails SILENTLY at runtime: the engine stores it happily, and then
 * `interpolateVars` — which only matches `[a-zA-Z0-9_]` — never finds it
 * again, so `{{vars.buyer type}}` renders as an empty string in the
 * summary the customer receives. Rejecting it at save time is the only
 * point where the author is still around to fix it.
 *
 * Shared by both node types rather than duplicated, so the rule cannot
 * drift between them.
 */
function validateOptionalVarKey(
  node_key: string,
  var_key: string | undefined
): ValidationIssue[] {
  const trimmed = var_key?.trim();
  if (!trimmed) return [];
  if (!VAR_KEY_PATTERN.test(trimmed)) {
    return [
      {
        severity: 'error',
        scope: 'node',
        node_key,
        field: 'var_key',
        message: `var_key "${trimmed}" must be alphanumeric+underscore and start with a letter or underscore, otherwise {{vars.${trimmed}}} will silently render as empty text.`,
      },
    ];
  }
  return [];
...
function validateNode(
  ...
  switch (node.node_type) {
    ...
    case 'send_buttons': {
      ...
      // Optional, but a malformed one fails SILENTLY at runtime — the
      // interpolator only matches [a-zA-Z0-9_], so "buyer type" would
      // store fine and then render as an empty string in the summary.
      // Caught here instead, where the author can still see it.
      issues.push(...validateOptionalVarKey(node.node_key, cfg.var_key));
      if (!cfg.text?.trim()) {
        issues.push({
          severity: 'error',
          scope: 'node',
    ...
    case 'collect_input': {
      ...
      } else if (!VAR_KEY_PATTERN.test(cfg.var_key)) {
        issues.push({
          severity: 'error',
          scope: 'node',
          node_key: node.node_key,
    ...
    case 'ai_agent': {
      ...
      if (!cfg.next_node_key) {
        issues.push({
          ...
          field: 'next_node_key',
          message:
            'AI Agent must point to a next node (usually a Handoff or End).',
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: 'error',
