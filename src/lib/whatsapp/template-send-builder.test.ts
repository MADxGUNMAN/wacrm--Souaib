...
describe('buildSendComponents — opt-out quick replies', () => {
  it('attaches the reserved payload to a "Stop" quick reply', () => {
    // Without this the payload defaults to the button LABEL, so an account
    // that later edits its keyword list silently breaks a button sitting in
    // an already-approved template.
    const components = buildSendComponents(
      row({ buttons: [{ type: 'QUICK_REPLY', text: 'Stop' }] })
    );
    ...
    expect(components).toEqual([
      {
        ...
        index: '0',
        parameters: [{ type: 'payload', payload: 'REPLAI_OPT_OUT' }],
      },
    ]);
  });

  it('uses the right index when the opt-out button is not first', () => {
    const components = buildSendComponents(
      row({
        buttons: [
  ...
  it('leaves ordinary quick replies alone', () => {
    // The long-standing behaviour: no component, and Meta echoes the label
    // back as the payload. Changing that for every quick reply in the
    // product would be a far wider blast radius than this feature needs.
    expect(
      buildSendComponents(
        row({
          buttons: [
  ...
  it('does NOT guess at ambiguous labels like "End" or "Cancel"', () => {
    // These are plausible opt-out words AND plausible labels for something
    // else ("End quiz", "Cancel order"). Treating them as opt-outs would
    // unsubscribe somebody for cancelling an order — worse than the dead
    // button the reserved payload exists to fix.
    expect(
      buildSendComponents(
        row({
          buttons: [
            { type: 'QUICK_REPLY', text: 'End' },
  ...
  it('still lets an explicit caller override win', () => {
    const components = buildSendComponents(
      row({ buttons: [{ type: 'QUICK_REPLY', text: 'Stop' }] }),
      { buttonParams: { 0: 'CUSTOM_PAYLOAD' } }
    );

    expect(components[0]).toMatchObject({
      parameters: [{ type: 'payload', payload: 'CUSTOM_PAYLOAD' }],
    });
  });
});
