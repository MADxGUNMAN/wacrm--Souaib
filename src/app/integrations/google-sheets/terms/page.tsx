...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title =
    page?.seo_meta_title ||
    `Terms of Service — Replai for Google Sheets Add-on`;
  const description =
    page?.seo_meta_description ||
    'Terms of Service and Acceptable Use Policy for the Replai Google Sheets Add-on.';
...
export default async function GoogleSheetsTermsPage() {
  const page = await getIntegrationPage('google-sheets');
  ...
  const markdownContent =
    page.terms_markdown ||
    `# Terms of Service — Replai for Google Sheets Add-on\n\nNo terms of service content configured yet in the Super Admin CMS.`;
        ...
        <div className="mb-10 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-10">
          <div className="mb-3 flex items-center gap-3 text-emerald-600">
            ...
            <span className="text-xs font-bold tracking-wider text-emerald-700 uppercase">
              Google Workspace Add-on Terms
            </span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">
            Terms of Service for Google Sheets Add-on
          </h1>
          <p className="mt-3 text-sm text-slate-500">
            End-user license terms, acceptable use requirements, service
            commitments, and WhatsApp policy compliance for Replai for Google
            Sheets.
          </p>
