...
import { Bell, CheckCheck, Gauge, Loader2, UserPlus } from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  const handleClick = useCallback(
    (n: Notification) => {
      ...
      // Conversation first, then the explicit link (migration 079).
      // Notifications that are not about a conversation used to be inert
      // when clicked, which made them read as broken rather than
      // informational.
      if (n.conversation_id) {
        router.push(`/inbox?c=${n.conversation_id}`);
        return;
      }
      // Internal paths only. A notification is written server-side today,
      // but treating the value as a URL would make any future writer one
      // mistake away from an open redirect.
