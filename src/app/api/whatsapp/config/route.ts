...
export async function POST(request: Request) {
  try {
    ...
    // Step 3: mirror the business profile picture.
    //
    // Best-effort. This is decoration — knowing which brand identity is on
    // the number — so a failure here must never cost the user a working
    // connection they just authenticated. The refresh button in WhatsApp
    // Setup exists precisely so this can be retried without re-saving
    // credentials.
    let businessProfile: { pictureUrl: string | null; about: string | null } = {
      pictureUrl: null,
      about: null,
    };
    if (!registrationError) {
      try {
        businessProfile = await syncBusinessProfile({
          phoneNumberId: phone_number_id,
          accessToken: access_token,
          accountId,
        });
        ...
        console.warn(
          'business profile sync failed (non-fatal):',
    ...
    const baseRow = {
      phone_number_id,
      // From the verifyPhoneNumber call above, which already had to
      // succeed for this save to proceed. Storing it is what lets the UI
      // show a number instead of the phone_number_id.
      ...
      // Our mirrored S3 URL, never Meta's — theirs is signed and expires
      // within days. See src/lib/whatsapp/business-profile.ts.
      business_profile_picture_url: businessProfile.pictureUrl,
      business_about: businessProfile.about,
      business_profile_synced_at: businessProfile.pictureUrl
