...
async function processMessage(
  ...
  //   text, image, document, audio, video, location, template, contact_card
  ...
  switch (message.type) {
    ...
    case 'contacts':
      if (message.contacts?.length) {
        const contactSummaries = message.contacts.map((sharedContact) => {
          const name =
            sharedContact.name?.formatted_name ||
            [sharedContact.name?.first_name, sharedContact.name?.last_name]
              .filter(Boolean)
              .join(' ');
          const phone =
            sharedContact.phones?.[0]?.phone ||
            sharedContact.phones?.[0]?.wa_id;
          return [name, phone].filter(Boolean).join(' - ');
        });

        return {
          contentText:
            contactSummaries.filter(Boolean).join('\n') || 'Contact shared',
          mediaUrl: null,
          mediaType: null,
        };
      }
