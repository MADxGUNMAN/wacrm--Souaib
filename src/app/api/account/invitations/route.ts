...
//
//   Both owner-only. The list endpoint is what the Members tab uses to
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as
      | { expiresInDays?: unknown; label?: unknown }
      | null;

    // All invitations create members — role is always 'member'.
